...
var underbarServerURL = 'http://oi-underbar.ifactory.com/underbar/';
// var underbarServerURL = 'http://oi-dev.ifactory.com/oiUnderbar/';
// var underbarServerURL = 'http://localhost:8080/underbar/';

var resourceURL = underbarServerURL;

var relatedContentURL;
var prefixRelatedContentURL = 'http://oxfordindex.oup.com/';
...
accept the optional width parameter and kick-off the process of rendering the OIUnderbar
*/
function PF_doInsertOIUnderbar(width, justify){
...
function callAjax(url, query, defaultQuery, preferredType, productCode, f){
	...
	url += "?qc=" + toURLParam(query) + "&dqc=" + toURLParam(defaultQuery) + "&pct=" + toURLParam(preferredType) + "&pid=" + toURLParam(productCode);
...
function submitSearch(){
	...
	var url = prefixRelatedContentURL + "search?q=" + getURLfromPrefix();
...
function fncOnBlurRelatedURL(){
	// var value = getURLfromPrefix();
	var splitQuery = underbar_splitQueryString(qc_query);
	var value = splitQuery.query; 
	if ("DOI" == splitQuery.context){
		relatedContentURL = prefixRelatedContentURL + "search?assoc=" + value;
	}
	if ("KEYW" == splitQuery.context){
		relatedContentURL = prefixRelatedContentURL + "search?f_0=keyword&q_0=" + value;
	}
	if ("SUBJ" == splitQuery.context){
		relatedContentURL = prefixRelatedContentURL + "search?t1=" + value;
	}
	if ("QS" == splitQuery.context){
		relatedContentURL = prefixRelatedContentURL + "search?q=" + value;
	}
...
function createLayout(){		
	...
	jQuery("<div></div>").attr('id','underbar-wrapper').appendTo('body');	
...
function createSearchItems(){
	...
	jQuery.each(resultLst, function (i){
		...
		jQuery('#content' + i).append("<h3>"+ resultLst[i].contentType +"</h3>");
		...
		if (resultLst[i].IndexCardSearchResults != ""){		
			...
			jQuery('#col' + i).append("<a href='" + morContentTypeLink + "' target='_blank' onclick='checkOpenURL(event);' style='text-decoration: none'><span class='underbar-more'>More "+ plural(resultLst[i].contentType) +" &raquo;</span></a>");			
