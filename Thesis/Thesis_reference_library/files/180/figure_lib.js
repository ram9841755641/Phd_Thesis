...
$(document).ready(function() {
	var expand_els = $('#col-3 div.expandable-block')
	if (expand_els[0]) {
		$('#col-3').addClass('js-target'); 
		//collapse blocks
...
$(document).ready(function() {
	if ($('#content-block div.fulltext-view div.pos-float.fig')[0]) {
		...
		if ($('#cb-art-related')[0]) {	
			...
			// $('#article-supplemental div.cb-contents').prepend(new_el);
			 $('#sidebar-global-nav').prepend(new_el);
		}
		...
		$('#col-3 div.dynamic-figs div.fig').eq(0).show();
		// if only one figure present remove navigation
		if (!$('#content-block div.fulltext-view div.fig')[1]) {
			$('#col-3 ul.fig-nav').remove();
		}
		//add 'active' class to first tab
		var nav_item = $('#col-3 ul.fig-nav li');
...
//event handlers - all bubble from #col-3
$(document).ready(function() {
	...
	target_area.click(function(e) { 
		...
		if(this_target.is('h4.cb-section-header')) { 
			...
			$('#col-3 div.expandable-block').filter(function () {
		...
		//figure viewer navigation
			...
			$('#col-3 ul.fig-nav li').removeClass('active')
			.filter(function () {
				return $(this).attr("id") == this_tab_id;
			})
			.addClass('active');
			$('#col-3 div.dynamic-figs div.fig').hide()
			.filter(function () {
...
function buildWin (link) {
	...
	var fig_txt = link.parents('div.fig').find('div.fig-caption').html();
	new_win.append('<div class="fig-caption">' + fig_txt + '</div>');
