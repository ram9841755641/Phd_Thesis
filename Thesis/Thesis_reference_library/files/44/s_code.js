...
/************************ REVISION HISTORY ************************
03-06-2013 	Updated to track the Article Title for PDF and EPUB downloads.
			Updated light server call tracking.
*/
...
function s_doPlugins(s) {
	s.dynComp('u');
	s.prop21 = 'V030513A';  //Code Version
	...
	if(document.location.pathname.indexOf('results.aspx')>-1 && s.c_r('s_q')!=document.location.search){	
		...
		if(navigator.appVersion.indexOf("MSIE")==-1){			
			if(document.getElementById('ej-box-01-body')!=null){
				s.prop4 = document.getElementById('ej-box-01-body').childNodes[3].children[2].innerHTML;
			}
		}
		else{
			if(document.getElementById('ej-box-01-body')!=null){
				s.prop4 = document.getElementById('ej-box-01-body').children[1].innerText.match('(.*) results')[1]						
			}
		}
...
/*		if(document.location.host.indexOf('networks')==-1){
	...
	if(s.getQueryParam("ET_CID")!=""){
		s.campaign=s.getQueryParam("ET_CID"); //ET Campaign ID
		s.eVar2=s.getQueryParam("ET_RID"); //ET Recipient ID
	}
...
s.socialTrack = function(t){	
	...
	s.linkTrackVars='events,eVar18,eVar27,prop20';
	s.atb = true;
	s.tl(true,'o',t);
	s.events=oldEvents;
