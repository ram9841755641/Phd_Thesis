...
var Outline = {
  ...
  addItemMMC: function(hashId, lbl, ext, hStr) {
    ...
    if(SDM.entitled==true){
      html = "<li><div id='ol_" + hashId + "' class='tocLink io item " + hStr + "sec mmc'>"
           + "<div class=\"olIcon " + ecn + "\"></div><a class=\"olIcon " + ecn + "\" href='#" + hashId + "' onClick=\"SD_UTIL.sendUserKeyEvent('displayTOCSection', 'article', 'leftPane', '" + SDM.keOriginContentFamily + "'); \">"
           + lbl + "</a></div></li>";
    }
    else{
      html = "<li><div id='ol_" + hashId + "' class='io item " + hStr + "sec mmc'>"
           + "<div class=\"olIcon " + ecn + "\"></div><a class=\"olIcon " + ecn + "  cLink\"  queryStr='" + SDM.urlTOCLinkQueryStr + "' href='" + SDM.urlTOCLink + "'\>"
           + lbl 
           + "</a></div></li>";
    }
    return html;
  ...
  addItemTbl: function(hashId, lbl, hStr) {
    var html = "<li><div id='ol_" + hashId + "' class='io item " + hStr + "sec tbl'>"
             + "<div class=\"olIcon olIconTbl\"></div><a class=\"tocLink olIcon olIconTbl\" href='#" + hashId + "' onClick=\"SD_UTIL.sendUserKeyEvent('displayTOCSection', 'article', 'leftPane', '" + SDM.keOriginContentFamily + "'); \">"
             + lbl + "</a></div></li>";
    return html;
...
$.fn.lazyLoadImages = function(opts) {
  ...
  function scrollAction() {
    ...
    for(i=0;i<totalCount && loopBool==true ;i++){
      ...
      if( Math.abs($('#centerPane').scrollTop()+$(window).height() - $(obj).positionA('#centerInner').top+$(obj).height()) < 
                                                                     $(window).height() + cfg.preLoadLine
       || Math.abs($('#centerPane').scrollTop() - $(obj).positionA('#centerInner').top)  < $(window).height() + cfg.preLoadLine) {
        var imgObj = $(obj);
	    ...
	    if(imgObj.closest('.formula')[0] && imgObj.width()>=595){
          ...
          if($('#centerPane').scrollTop()+$(window).height() - $(obj).positionA('#centerInner').top+$(obj).height() < 
             $(window).height() + cfg.preLoadLine && scrollDown){ 
	      loopBool=false;
          }
          if($('#centerPane').scrollTop() - $(obj).positionA('#centerInner').top  <  $(window).height() + cfg.preLoadLine && scrollUp){
              loopBool=false;
          }
      }
    }
    ...
    populateFigures:function(selObj,selObjClass){
        ...
        if($(selObj).find('dl.figure').length>0){
            $(selObj).find('dl.figure').each(function() {
                if($(this).find("img.figure").attr("data-fullEID")==undefined){
                    imgSrc= SDM.imageUrlPrefix + $(this).find("img.figure").attr("data-gabsEID");
                }else{
                    imgSrc= SDM.imageUrlPrefix + $(this).find("img.figure").attr("data-fullEID");
                }
            ...
            $.each ($(selObj).find("img.figure"), function(){
                if($(this).attr("data-fullEID")==undefined){
    ...
    var marginLevel = SDU.isIE7()?700:300;
        if(mathRenderer.opt.renderOn){
            ...
            for(i=0;i<totalCount && loopBool==true ;i++){
                  ...
                  var pos = $(obj).positionA('#centerInner'),
...
var EReader = {
  ...
  errorLoader: function($e, cfg) {
    if(cfg.msgType == 'rect') {
      ...
      $('#ldr' + cfg.name).append('<div class="msg err">' + cfg.name + ' still generating. Please try again later.</div>');
    }
  },
