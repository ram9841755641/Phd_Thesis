...
site.initialiseScrollable = function() {
    ...
    if(gall.length == 1) {
        $(".scrollable-container .navi, .scrollable-container .browse").detach();
    }
...
site.parseCookie = function(cookie) {
...
site.welcomeMessage = function() {

   

    var cookie = site.allCookies.getItem("bmccookie");

    var obj = (cookie != null) ? site.parseCookie(cookie) : {};
        ...
        if(obj.hasOwnProperty("fname")) {
            ...
            welcome = obj.fname.replace("+", " ") + " " + obj['lname'].replace("+"," ");

            
            $(".login span#username").html(welcome);
            $(".off").show();


        } else if(obj.hasOwnProperty("institution_name")) {

            welcome = obj["institution_name"].replace("+"," ");
             $("#login #username").append(welcome);
...
jQuery(document).ready(function() {
    ...
    if(jQuery("#articleSignupForUpdates").length > 0) {
        jQuery("#articleSignupForUpdates").submit(function(e) {
            e.preventDefault();
            site.submitnewsletter({address: $(this).find("input#email").attr("value"), newsletters: ["bmcupdates","JOURNAL_SPECIFIC_NEWSLETTER"], action:true, messagecontainer: "#message-box"} );
        });
    }
    ...
    if ((site.id == "3008") && site.page == "home") {
...
site.trendingArticles = function(options) {
    ...
    $.ajax({url:options.uri, timeout: 6000, type: "GET", dataType: "jsonp", cache: true})
    .error(function(event, request, settings){ 

        $( "#error-message" ).append("<div class='msg msg-err'><div class='msg-err-img'></div><div class='msg-text'>Error requesting data from altmetric. Please try again later. </div>");
        $( "#loading" ).hide();
    ...
    function render(entries) {
            ...
            var html = '';
                ...
                //for(var i = 0; i <= entries.length; i++) {
                     ...
                     html += '<a href="http://' + entries.url + '/about">Accesses</a> | ' + '<a href="http://' + entries.url + '/about#citations">Citations</a>';
                     html += '</div></li>'; 
